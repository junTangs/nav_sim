REWARD_FACTORY = {}
from nav_sim.utils.math_utils import distance,gaussian,manhattan_distance
...
@njit
def att_filed_manhattan(x1,y1,x2,y2,a):
    return 0.5*a*manhattan_distance(x1,y1,x2,y2)**2

@njit
def rep_filed_manhattan(x1,y1,x2,y2,r,a):
...
def cal_apf_manhattan(goals,obstacles,humans,robot):
    att = 0
    rep = 0
    for g in goals:
         att += att_filed(g.x,g.y,robot.x,robot.y,1)
...
def sparse_reward_manhattan(instance,*args,**kwargs):
    collide = instance.collide_flag
    finish = instance.finish_flag
...
REWARD_FACTORY["sparse_manhattan"] = sparse_reward_manhattan